  ...
  onSubmit() {
    ...
    this.authService.login(user).subscribe(response => {
      ...
      if (response['JWTTOKEN']) {
        ...
        this.snackBar.open(`UserId ${user.userId} LoggedIn Successfully. `, '', { duration: 1000 })
          .afterDismissed().subscribe(() => {
            console.log(`Snackbar Dismissed,Routing To Popular Page.`);
            this.router.navigate(['/movies/popular']);
        });
      }else{
        this.snackBar.open(`UserId ${user.userId} Unable To Login Successfully. `, '', { duration: 1000 })
          .afterDismissed().subscribe(() => {
